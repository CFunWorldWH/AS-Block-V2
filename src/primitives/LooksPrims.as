...
//		primTable['yScroll']				= function(b:*):* { return app.stagePane.yScroll };
	...
	private function showBubbleAndWait(b:Block, type:String):void {
		...
		if (interp.activeThread.firstTime) {
			...
			//arg和numarg要分开处理_wh
			if(interp.activeThread.comWaitNum == 0)//对于读取模块，由于需要循环等待，过程中不显示_wh
			{
				if(interp.activeThread.ArduinoNA)//加有效性判断_wh
					interp.activeThread.ArduinoNA = false;
				else
				{
	...
	private function primChangeSize(b:Block):void {
		...
		var size:Number = interp.numarg(b, 0);
		if(app.interp.activeThread.ArduinoNA)//加有效性判断_wh
			app.interp.activeThread.ArduinoNA = false;
		else
		{
			s.setSize(s.getSize() + size);
	...
	private function primGoBack(b:Block):void {
		var s:ScratchSprite = interp.targetSprite();
		if ((s == null) || (s.parent == null)) return;
		var index:Number = interp.numarg(b, 0);
		if(app.interp.activeThread.ArduinoNA)//加有效性判断_wh
			app.interp.activeThread.ArduinoNA = false;
		else
		{
			var newIndex:int = s.parent.getChildIndex(s) - index;
