...
public class MotionAndPenPrims {
	...
	private function primTurnRight(b:Block):void {
		var s:ScratchSprite = interp.targetSprite();
		var right:Number = interp.numarg(b, 0);
		if(app.interp.activeThread.ArduinoNA)//加有效性判断_wh
			app.interp.activeThread.ArduinoNA = false;
		else
		{
			if (s != null) s.setDirection(s.direction + right);
	...
	private function primTurnLeft(b:Block):void {
		var s:ScratchSprite = interp.targetSprite();
		var left:Number = interp.numarg(b, 0);
		if(app.interp.activeThread.ArduinoNA)//加有效性判断_wh
			app.interp.activeThread.ArduinoNA = false;
		else
		{
			if (s != null) s.setDirection(s.direction - left);
			if (s.visible) interp.redraw();
		}
	}
	...
	private function primSetDirection(b:Block):void {
		var s:ScratchSprite = interp.targetSprite();
		var dir:Number = interp.numarg(b, 0);
		if(app.interp.activeThread.ArduinoNA)//加有效性判断_wh
			app.interp.activeThread.ArduinoNA = false;
		else
		{
	...
	private function primGoTo(b:Block):void {
		var s:ScratchSprite = interp.targetSprite();
		var x:Number = interp.numarg(b, 0);
		var y:Number = interp.numarg(b, 1);
		if(app.interp.activeThread.ArduinoNA)//加有效性判断_wh
			app.interp.activeThread.ArduinoNA = false;
		else
			if (s != null) moveSpriteTo(s, x, y);
	}
	...
	private function primChangeX(b:Block):void {
		...
		if(app.interp.activeThread.ArduinoNA)//加有效性判断_wh
			...
			if (s != null) moveSpriteTo(s, s.scratchX + x, s.scratchY);
	}
	...
	private function primSetX(b:Block):void {
		...
		if(app.interp.activeThread.ArduinoNA)//加有效性判断_wh
			...
			if (s != null) moveSpriteTo(s, x, s.scratchY);
	}

	private function primChangeY(b:Block):void {
		var s:ScratchSprite = interp.targetSprite();
		var y:Number = interp.numarg(b, 0);
		if(app.interp.activeThread.ArduinoNA)//加有效性判断_wh
			app.interp.activeThread.ArduinoNA = false;
		else
			if (s != null) moveSpriteTo(s, s.scratchX, s.scratchY + y);
	}
	...
	private function primSetY(b:Block):void {
		...
		if(app.interp.activeThread.ArduinoNA)//加有效性判断_wh
			...
			if (s != null) moveSpriteTo(s, s.scratchX, y);
	}
	...
	private function primSetPenHue(b:Block):void {
		...
		if (s != null)
		{
			var clo:Number = interp.numarg(b, 0);
			if(app.interp.activeThread.ArduinoNA)//加有效性判断_wh
				app.interp.activeThread.ArduinoNA = false;
			else
				s.setPenHue(clo);
	...
	private function primSetPenShade(b:Block):void {
		...
		if (s != null)
		{
			var sha:Number = interp.numarg(b, 0);
			if(app.interp.activeThread.ArduinoNA)//加有效性判断_wh
				app.interp.activeThread.ArduinoNA = false;
			else
				s.setPenShade(sha);
	...
	private function primSetPenSize(b:Block):void {
		...
		if (s != null)
		{
			...
			if(app.interp.activeThread.ArduinoNA)//加有效性判断_wh
				...
				s.setPenSize(Math.max(1, Math.min(960, Math.round(interp.numarg(b, 0)))));
		}
	}
	...
	private function primChangePenSize(b:Block):void {
		...
		if (s != null)
		{
			var siz:Number = interp.numarg(b, 0);
			if(app.interp.activeThread.ArduinoNA)//加有效性判断_wh
				app.interp.activeThread.ArduinoNA = false;
			else
				s.setPenSize(s.penWidth + siz);
		}
	}
