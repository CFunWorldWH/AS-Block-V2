...
public class SoundPrims {
	...
	private function primPlayNote(b:Block):void {
		...
		if (s == null) return;
		var a:Number = interp.numarg(b, 0);
		var bb:Number = interp.numarg(b, 1);
		if(app.interp.activeThread.ArduinoNA)//加有效性判断_wh
			app.interp.activeThread.ArduinoNA = false;
		else
		{
			if (interp.activeThread.firstTime) {
				var key:Number = a;
				var secs:Number = beatsToSeconds(bb);
	...
	private function primPlayRest(b:Block):void {
		...
		{
			if (interp.activeThread.firstTime) {
				var secs:Number = beatsToSeconds(a);
				interp.startTimer(secs);
			} else {
				interp.checkTimer();
			}
	...
	private function primChangeVolume(b:Block):void {
		var s:ScratchObj = interp.targetObj();
		if (s != null) {
			var volume:Number = interp.numarg(b, 0);
			if(app.interp.activeThread.ArduinoNA)//加有效性判断_wh
				app.interp.activeThread.ArduinoNA = false;
			else
			{
				s.setVolume(s.volume + volume);
				interp.redraw();
			}
		}
	}
